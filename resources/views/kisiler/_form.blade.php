@php
    ...
    /** @var \App\Models\Kisi|null $kisi */
    $kisi = $kisi ?? null;

    $val = function (string $key, mixed $default = null) use ($kisi) {
        if (old($key) !== null) {
            return old($key);
        }
        ...
        $value = data_get($kisi, $key);

        return $value ?? $default;
    };

    $dateVal = function (string $key) use ($kisi) {
        if (old($key) !== null) {
            return old($key);
        }
    ...
    $currentCinsiyet = old('cinsiyet') ?? $kisi?->cinsiyet?->value ?? '';
@endphp
...
>
    ...
    <div class="card form-section-card">
        <div class="card-section-header">
            <div>
                <h2 class="card-section-title">İletişim Bilgileri</h2>
                <p class="card-section-desc">Telefon, e-posta ve adres bilgileri.</p>
            </div>
            <button
                type="button"
                class="btn-kimlik-sorgula"
        ...
        <div class="form-grid">
            ...
            <div class="form-group">
                ...
                <input type="email" id="email" name="email" value="{{ $val('email') }}" class="form-control @error('email') is-invalid @enderror" maxlength="150" autocomplete="off">
            </div>
