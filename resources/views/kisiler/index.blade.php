...
<div class="page-header">
    <div class="page-header-text">
        ...
        <p class="page-subtitle">Kişileri listeleyin, oluşturun ve düzenleyin.</p>
    </div>
    <div class="flex items-center gap-2">
        <x-cta-button :href="route('kisiler.create')" icon="plus">Yeni Kişi</x-cta-button>
    </div>
</div>
...
<div class="card filter-card">
    <div class="card-section-header">
        <div>
            ...
            <p class="card-section-desc">Ad soyad, TC kimlik no, telefon, e-posta, cinsiyet ve duruma göre daraltın.</p>
        </div>
    </div>

    <form id="kisiler-filter-form" method="GET" action="{{ route('kisiler.index') }}">
        <div class="filter-grid">
            ...
            <div class="form-group">
                ...
                <select id="cinsiyet" name="cinsiyet" class="form-control" data-reset-value="tumu">
                    <option value="tumu" @selected(($filters['cinsiyet'] ?? 'tumu') === 'tumu')>Tümü</option>
                    @foreach (\App\Enums\Cinsiyet::cases() as $cinsiyet)
                        <option value="{{ $cinsiyet->value }}" @selected(($filters['cinsiyet'] ?? 'tumu') === $cinsiyet->value)>{{ $cinsiyet->label() }}</option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="filter-actions">
            <div class="filter-actions-right">
                <x-back-button id="kisiler-filter-clear" type="button" icon="close">Temizle</x-back-button>
...
{{-- Tablo Kartı --}}
<div class="card table-card" id="kisiler-table-card">
    <div class="table-toolbar">
        <div>
            <div class="table-title">Kişi Kayıtları</div>
        ...
        <div class="flex items-center gap-2.5">
            ...
            <x-excel-export
                :href="route('kisiler.export', request()->query())"
                id="kisiler-excel-link"
            />
        </div>
    </div>

    <div id="kisiler-results">
        @include('kisiler._results')
    </div>
</div>
@endsection
