@php
    ...
    $sortableColumns = ['ad', 'baslangic', 'bitis', 'grup_sayisi', 'olusturma'];
@endphp

<div class="table-wrapper">
    <table
        class="data-table"
        id="kres-donemler-table"
    ...
    >
        ...
        <tbody>
            @forelse ($donemler as $donem)
                <tr>
                    <td data-column="ad">{{ $donem->ad }}</td>
                    <td data-column="baslangic">{{ $donem->baslangic?->format('d.m.Y') ?? '—' }}</td>
                        ...
                        <div class="row-actions" data-row-actions>
                            ...
                            <div class="action-dropdown" data-action-dropdown hidden role="menu">
                                <button
                                    ...
                                    role="menuitem"
                                    data-entity-edit
                                    data-update-url="{{ route('kres.donemler.update', $donem) }}"
                                    data-ad="{{ $donem->ad }}"
                ...
                <tr>
                    <td colspan="{{ count($allColumns) }}">
                        <div class="empty-state">
                            ...
                            <div class="empty-state-title">Dönem kaydı bulunamadı</div>
...
<div class="table-footer">
    <div style="display:flex; align-items:center; gap:12px; font-size:13px; color:#7e8299;">
        <span>
            @if ($donemler->total())
                {{ $donemler->firstItem() }}–{{ $donemler->lastItem() }} / {{ $donemler->total() }} kayıt
            @else
                0 kayıt
            @endif
        </span>
