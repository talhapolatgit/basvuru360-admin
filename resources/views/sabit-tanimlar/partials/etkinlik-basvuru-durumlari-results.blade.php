@php
    ...
    $sortableColumns = ['sira', 'kod', 'ad'];
@endphp

<div class="table-wrapper">
    <table
        class="data-table"
        id="etkinlik-basvuru-durumlari-table"
        ...
        <tbody>
            @forelse ($items as $item)
                <tr>
                    ...
                    <td data-column="kod"><code class="sabit-kod">{{ $item->kod }}</code></td>
                    <td data-column="ad">
                        <div>{{ $item->ad }}</div>
                        @if ($item->aciklama)
                            <div class="sabit-row-hint">{{ $item->aciklama }}</div>
                        @endif
                    </td>
                    <td data-column="status">
                        <span class="status {{ $item->statusClass() }}">{{ $item->ad }}</span>
                    </td>
                        ...
                        <div class="row-actions" data-row-actions>
                            ...
                            <div class="action-dropdown" data-action-dropdown hidden role="menu">
                                ...
                                <button
                                    ...
                                    role="menuitem"
                                    data-etkinlik-basvuru-durumlari-edit
                                    data-update-url="{{ route('sabit-tanimlar.etkinlik-basvuru-durumlari.update', $item) }}"
                                    data-kod="{{ $item->kod }}"
                                    data-ad="{{ $item->ad }}"
                                    data-aciklama="{{ $item->aciklama }}"
                                    data-status-sinifi="{{ $item->status_sinifi }}"
                                    data-sira="{{ $item->sira }}"
                ...
                <tr>
                    <td colspan="{{ count($allColumns) }}">
                        <div class="empty-state">
                            <div class="empty-state-title">Etkinlik başvuru durumu bulunamadı</div>
