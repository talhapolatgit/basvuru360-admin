...
<div class="page-header">
    <div class="page-header-main">
        <div
            ...
            data-avatar-preview
            data-upload-url="{{ route('egitmenler.foto.update', $egitmen) }}"
            data-delete-url="{{ route('egitmenler.foto.delete', $egitmen) }}"
        >
            <label class="profil-avatar profil-avatar-lg profil-avatar-edit" title="Yeni fotoğraf yükle">
                @if ($egitmen->profil_foto_url)
                    <img src="{{ $egitmen->profil_foto_url }}" alt="Profil fotoğrafı" data-avatar-img>
                    <span class="profil-avatar-initials" data-avatar-initials hidden>{{ $egitmen->bas_harfler ?: '?' }}</span>
                @else
                    <img src="" alt="Profil fotoğrafı" data-avatar-img hidden>
                    <span class="profil-avatar-initials" data-avatar-initials>{{ $egitmen->bas_harfler ?: '?' }}</span>
                @endif
            ...
            <button
                ...
                @unless ($egitmen->profil_foto) hidden @endunless
            >
        ...
        <div class="page-header-text">
            <p class="page-eyebrow">Eğitmenler</p>
            <h1 class="page-title">{{ $egitmen->tam_adi }}</h1>
            <p class="page-subtitle">Eğitmen bilgilerini görüntüleyin ve atandığı kursları inceleyin.</p>
        </div>
    </div>
    <div class="flex items-center gap-2">
        <x-back-button href="{{ route('egitmenler.index') }}">Eğitmenlere Dön</x-back-button>
    </div>
</div>
...
<div class="card table-card">
    <div class="table-toolbar">
        <div>
            <div class="table-title">Eğitmen Bilgileri</div>
    ...
    <div class="lesson-info-grid" style="margin-bottom: 0;">
        <div class="lesson-info-card">
            ...
            <div class="lesson-info-value">{{ $egitmen->tc_kimlik_no ?: '—' }}</div>
        </div>
        <div class="lesson-info-card">
            <div class="lesson-info-label">Doğum Tarihi</div>
            <div class="lesson-info-value">{{ $egitmen->dogum_tarihi?->format('d.m.Y') ?? '—' }}</div>
        </div>
        <div class="lesson-info-card">
            <div class="lesson-info-label">Cinsiyet</div>
            <div class="lesson-info-value">{{ $egitmen->cinsiyet?->label() ?? '—' }}</div>
        </div>
        <div class="lesson-info-card">
            <div class="lesson-info-label">Doğum Yeri</div>
            <div class="lesson-info-value">{{ $egitmen->dogum_yeri ?: '—' }}</div>
        </div>
        <div class="lesson-info-card">
            <div class="lesson-info-label">Medeni Durum</div>
            <div class="lesson-info-value">{{ $egitmen->medeni_durum ?: '—' }}</div>
        </div>
        <div class="lesson-info-card">
            <div class="lesson-info-label">Uyruk</div>
            <div class="lesson-info-value">{{ $egitmen->uyruk ?: '—' }}</div>
        </div>
        <div class="lesson-info-card">
            <div class="lesson-info-label">Anne Adı</div>
            <div class="lesson-info-value">{{ $egitmen->anne_adi ?: '—' }}</div>
        </div>
        <div class="lesson-info-card">
            <div class="lesson-info-label">Baba Adı</div>
            <div class="lesson-info-value">{{ $egitmen->baba_adi ?: '—' }}</div>
        </div>
        <div class="lesson-info-card">
            <div class="lesson-info-label">Telefon</div>
            <div class="lesson-info-value">{{ $egitmen->telefon ?: '—' }}</div>
        </div>
        <div class="lesson-info-card">
            <div class="lesson-info-label">E-posta</div>
            <div class="lesson-info-value">{{ $egitmen->email }}</div>
        </div>
        <div class="lesson-info-card">
            <div class="lesson-info-label">İl / İlçe</div>
            <div class="lesson-info-value">
                @if ($egitmen->il || $egitmen->ilce)
                    {{ trim(($egitmen->il ?? '').' / '.($egitmen->ilce ?? ''), ' /') }}
                @else
                    —
                @endif
            </div>
        ...
        <div class="lesson-info-card">
            ...
            <div class="lesson-info-value">{{ $egitmen->adres ?: '—' }}</div>
        </div>
        <div class="lesson-info-card">
            <div class="lesson-info-label">Kayıt Tarihi</div>
            <div class="lesson-info-value">{{ $egitmen->created_at?->format('d.m.Y') }}</div>
        </div>
    </div>
</div>
...
<div class="lesson-actions-block">
    <div class="lesson-actions-card">
        <div class="lesson-actions-card-head">
            ...
            <p class="lesson-actions-subtitle">Eğitmen bilgilerini düzenleyin; SMS, e-posta veya şifre gönderin</p>
        </div>
        <div class="lesson-actions-grid">
            <a href="{{ route('egitmenler.edit', $egitmen) }}" class="lesson-action-btn lesson-action-btn-primary">
                ...
                <span class="lesson-action-copy">
                    ...
                    <span class="lesson-action-hint">Eğitmen bilgilerini güncelle</span>
                </span>
            </a>
            <a href="{{ route('takvim.index', ['ogretmen' => $egitmen->id]) }}" class="lesson-action-btn lesson-action-btn-primary">
                <span class="lesson-action-icon" aria-hidden="true">
                ...
                <span class="lesson-action-copy">
                    ...
                    <span class="lesson-action-hint">Eğitmenin ders takvimini görüntüle</span>
                </span>
            </a>
            <button
                type="button"
            ...
            >
                ...
                <span class="lesson-action-copy">
                    ...
                    <span class="lesson-action-hint">{{ $telefonVar ? ($egitmen->telefon ?: 'Telefon') : 'Telefon yok' }}</span>
                </span>
            </button>
            <button
                type="button"
            ...
            >
                ...
                <span class="lesson-action-copy">
                    ...
                    <span class="lesson-action-hint">{{ $emailVar ? $egitmen->email : 'E-posta yok' }}</span>
                </span>
            </button>
            <button
                type="button"
    ...
    <div class="card table-card">
        <div class="table-toolbar">
            <div>
                ...
                <p class="table-subtitle">Bu eğitmenin atandığı tüm kurslar.</p>
            </div>
        ...
        <div class="table-wrapper">
            <table
                class="data-table"
                data-detail-table="egitmen_kurslar_cols"
                        ...
                        <tr>
                            <td colspan="9">
                                <div class="empty-state">
                                    ...
                                    <p class="empty-state-text">Bu eğitmene henüz bir kurs atanmamış.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    <div class="card table-card">
        <div class="table-toolbar">
            <div>
                ...
                <p class="table-subtitle">Eğitmenin atandığı kurslardaki tüm ders oturumları.</p>
            </div>
        ...
        <div class="table-wrapper">
            <table
                class="data-table"
                data-detail-table="egitmen_dersler_cols"
                        ...
                        <tr>
                            <td colspan="7">
                                <div class="empty-state">
                                    ...
                                    <p class="empty-state-text">Bu eğitmenin kurslarında oluşturulmuş ders oturumu yok.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
<div class="lesson-tab-panel" data-egitmen-panel="yoklamalar" role="tabpanel">
    <div class="card table-card">
        ...
        <div class="table-wrapper">
            <table
                class="data-table"
                data-detail-table="egitmen_yoklama_cols"
    ...
    <div class="card table-card">
        <div class="table-toolbar">
            <div>
                ...
                <p class="table-subtitle">Bu eğitmenin sorumlu olarak atandığı tüm etkinlikler.</p>
            </div>
        ...
        <div class="table-wrapper">
            <table
                class="data-table"
                data-detail-table="egitmen_etkinlikler_cols"
                        ...
                        <tr>
                            <td colspan="9">
                                <div class="empty-state">
                                    ...
                                    <p class="empty-state-text">Bu eğitmen henüz bir etkinliğe sorumlu olarak atanmamış.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    <div class="card table-card">
        <div class="table-toolbar">
            <div>
                ...
                <p class="table-subtitle">Bu eğitmenin gönderdiği tüm SMS ve e-posta kayıtları.</p>
            </div>
        </div>
                        ...
                        <tr>
                            <td colspan="5">
                                <div class="empty-state">
                                    ...
                                    <p class="empty-state-text">Bu eğitmen henüz SMS veya e-posta göndermemiş.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
<div class="confirm-modal" id="egitmen-sms-modal" hidden
    data-send-url="{{ route('egitmenler.sms.send', $egitmen) }}"
    data-ad="{{ $egitmen->tam_adi }}"
    ...
    <div class="confirm-modal-dialog confirm-modal-dialog-lg" role="dialog" aria-modal="true" aria-labelledby="egitmen-sms-modal-title">
        ...
        <div class="confirm-modal-body">
            ...
            <p class="form-hint" data-egitmen-sms-no-telefon hidden style="margin-bottom:14px; color:#f64e60;">
                Bu eğitmen için kayıtlı telefon numarası bulunamadı.
            </p>
...
<div class="confirm-modal" id="egitmen-eposta-modal" hidden
    data-send-url="{{ route('egitmenler.eposta.send', $egitmen) }}"
    data-ad="{{ $egitmen->tam_adi }}"
    data-email="{{ $egitmen->email }}"
    ...
    <div class="confirm-modal-dialog confirm-modal-dialog-lg" role="dialog" aria-modal="true" aria-labelledby="egitmen-eposta-modal-title">
        ...
        <div class="confirm-modal-body">
            ...
            <p class="form-hint" data-egitmen-eposta-no-email hidden style="margin-bottom:14px; color:#f64e60;">
                Bu eğitmen için kayıtlı e-posta adresi bulunamadı.
            </p>
...
<div class="confirm-modal" id="egitmen-sifre-modal" hidden
    data-send-url="{{ route('egitmenler.sifre.send', $egitmen) }}"
    data-ad="{{ $egitmen->tam_adi }}"
    data-telefon-var="{{ $telefonVar ? '1' : '0' }}"
    data-email-var="{{ $emailVar ? '1' : '0' }}"
>
