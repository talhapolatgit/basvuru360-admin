@php
    ...
    $sortableColumns = ['ad', 'alan', 'aktif_kurs_sayisi', 'olusturma'];
@endphp
    ...
    >
        ...
        <tbody>
            @forelse ($branslar as $brans)
                <tr>
                    <td data-column="ad">
                        <a href="{{ route('branslar.show', $brans) }}">{{ $brans->ad }}</a>
                    </td>
                    <td data-column="alan">{{ $brans->alan?->ad ?? '—' }}</td>
                    <td data-column="aktif_kurs_sayisi">{{ number_format($brans->aktif_kurs_sayisi) }}</td>
                    ...
                    <td data-column="olusturma">{{ $brans->created_at?->format('d.m.Y') }}</td>
                    <td data-column="islemler">
                        <div class="row-actions" data-row-actions>
                            ...
                            <div class="action-dropdown" data-action-dropdown hidden role="menu">
                                <a href="{{ route('branslar.show', $brans) }}" class="action-dropdown-item" role="menuitem">
                ...
                <tr>
                    <td colspan="{{ count($allColumns) }}">
                        <div class="empty-state">
                            ...
                            <div class="empty-state-title">Branş kaydı bulunamadı</div>
...
<div class="table-footer">
    <div style="display:flex; align-items:center; gap:12px; font-size:13px; color:#7e8299;">
        <span>
            @if ($branslar->total())
                {{ $branslar->firstItem() }}–{{ $branslar->lastItem() }} / {{ $branslar->total() }} kayıt
            @else
                0 kayıt
            @endif
        </span>
