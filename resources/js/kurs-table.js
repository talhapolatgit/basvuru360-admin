...
const COOKIE_KEY = 'kurslar_table_prefs';
...
function persistColumnPreferences() {
    const table = document.getElementById('kurslar-table');
    if (!table) {
        return false;
    }
...
function initSorting(table) {
    ...
    table.querySelectorAll('thead th[data-sortable="1"]').forEach((th) => {
        ...
        th.onclick = () => {
            ...
            loadKurslar(url.toString());
        };
    });
}

function updateExcelLink(url) {
    const excel = document.getElementById('kurslar-excel-link');
    if (!excel) {
        return;
    }
...
function setLoading(loading) {
    isLoading = loading;
    document.getElementById('kurslar-table-card')?.classList.toggle('is-loading', loading);
}

async function loadKurslar(url, { push = true } = {}) {
    if (isLoading) {
        return;
    }
    ...
    try {
        ...
        const results = document.getElementById('kurslar-results');
        if (results) {
...
function resetFilterForm(form) {
    ...
    form.querySelectorAll('select').forEach((select) => {
        const fallback = ['durum', 'yayinda'].includes(select.id) ? 'tumu' : '';
    ...
    const gunlerDisplay = document.getElementById('gunlerDisplay');
    if (gunlerDisplay) {
        gunlerDisplay.textContent = 'Tümü';
    }
...
const KURS_NO_MODE_KEY = 'kurs_no_search_mode_v2';
...
function initMoreFilters() {
    const form = document.getElementById('kurslar-filter-form');
    ...
    const setOpen = (open) => {
        isOpen = open;
        localStorage.setItem('kurslar_more_filters', open ? '1' : '0');
        applyVisibility();
    };
...
function initAjaxFilters() {
    ...
    form.addEventListener('submit', (event) => {
        event.preventDefault();
        loadKurslar(formToUrl(form));
    });

    document.getElementById('kurslar-filter-clear')?.addEventListener('click', () => {
        resetFilterForm(form);
        const url = new URL(form.action, window.location.origin);
        url.searchParams.set('durum', 'tumu');
        url.searchParams.set('yayinda', 'tumu');
        loadKurslar(url.toString());
    });

    window.addEventListener('popstate', () => {
        loadKurslar(window.location.href, { push: false });
    });
}

export function initKursTable() {
    // Detay sayfasındaki başvuru işlem menüsü de aynı handler'ı kullanır.
    initRowActions();

    if (!document.getElementById('kurslar-table')) {
        return;
    }
...
function positionRowActionDropdown(toggle, dropdown) {
    const rect = toggle.getBoundingClientRect();
    const menuWidth = Math.max(dropdown.offsetWidth || 168, 168);
    const estimatedHeight = dropdown.offsetHeight || 220;
    const spaceBelow = window.innerHeight - rect.bottom;
    const openUp = spaceBelow < estimatedHeight + 12 && rect.top > spaceBelow;

    dropdown.classList.toggle('is-dropup', openUp);
    dropdown.style.position = 'fixed';
    dropdown.style.right = 'auto';
    dropdown.style.left = `${Math.min(Math.max(8, rect.right - menuWidth), window.innerWidth - menuWidth - 8)}px`;
    ...
    if (openUp) {
        ...
        dropdown.style.top = `${Math.min(rect.bottom + 6, window.innerHeight - estimatedHeight - 8)}px`;
    }
}

function initRowActions() {
    if (document.documentElement.dataset.rowActionsBound === '1') {
        return;
    }
    document.documentElement.dataset.rowActionsBound = '1';

    const repositionOpenMenu = () => {
        const openWrap = document.querySelector('[data-row-actions].is-open');
        if (!openWrap) return;
        const toggle = openWrap.querySelector('[data-action-toggle]');
        const dropdown = openWrap.querySelector('[data-action-dropdown]');
        if (toggle && dropdown && !dropdown.hidden) {
            positionRowActionDropdown(toggle, dropdown);
        }
    };
    ...
    document.addEventListener('click', (event) => {
        ...
        if (toggle) {
            ...
            const wrap = toggle.closest('[data-row-actions]');
            const dropdown = wrap?.querySelector('[data-action-dropdown]');
            if (!wrap || !dropdown) {
                return;
            }
