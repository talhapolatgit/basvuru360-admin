...
return new class extends Migration
{
    ...
    {
        $tanim = collect(YetkiKatalogu::tanimlar())->firstWhere('kod', 'etkinlik_basvuru.olustur');
        if (! $tanim) {
            return;
        }

        $yetki = Yetki::query()->updateOrCreate(
            ['kod' => 'etkinlik_basvuru.olustur'],
            [
                'ad' => $tanim['ad'],
                'modul' => $tanim['modul'],
    ...
    {
        $yetki = Yetki::query()->where('kod', 'etkinlik_basvuru.olustur')->first();
        if ($yetki) {
            $yetki->roller()->detach();
            $yetki->delete();
        }
